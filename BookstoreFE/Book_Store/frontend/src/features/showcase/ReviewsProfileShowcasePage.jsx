...
import { BooksApi, ReviewsApi, CustomersApi } from '../../api/client';
...
export default function ReviewsProfileShowcasePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [reviewData, bookData, customerList] = await Promise.all([
          ReviewsApi.list(),
          BooksApi.list(),
          CustomersApi.list(),
        ]);

        const customerData = Array.isArray(customerList) ? customerList.find(c => Number(c.id) === Number(customerId || 1)) : null;
        ...
        if (customerData && customerData.id) {
          setProfileData({
            name: customerData.name || profileSummary.name,
            email: customerData.email || profileSummary.email,
            membership: 'Thanh vien Premium'
          });
          setProfileForm({
            name: customerData.name || profileSummary.name,
  ...
  async function saveProfile() {
    alert('Profile update is not supported by the backend yet.');
    // update state locally for demo purposes
          ...
          <div className="sb-profile-sidebar">
            <aside className="sb-profile-card sb-profile-card-showcase" id="profile-card">
              ...
              <button
                ...
                onMouseOver={e => e.currentTarget.style.background = 'rgba(255,77,79,0.07)'}
          ...
          <section className="sb-review-feed sb-review-feed-showcase">
            ...
            {activeSection === 'reviews' && pagedReviews.length ? pagedReviews.map((review) => (
              <article key={review.id} className="sb-review-card sb-review-card-showcase">
                <BookCover
                  book={bookForReview(review.book_id)}
                  style={{ width: '80px', height: '110px', objectFit: 'cover', borderRadius: '6px', flexShrink: 0 }}
                />
