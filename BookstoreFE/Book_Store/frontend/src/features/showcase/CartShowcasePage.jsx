...
import { BooksApi, CartApi, OrdersApi, getErrorMessage } from '../../api/client';
...
export default function CartShowcasePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const safeItems = Array.isArray(cartData) ? cartData : [];
        setBooks(safeBooks);
        setItems(safeItems);
      } catch (loadError) {
    ...
    () =>
      items.map((item) => {
        ...
        return {
          ...
          // Pass the full book object for BookCover
          display_image_url: book?.display_image_url || null,
          image_status: book?.image_status || 'READY',
        };
      }),
    [books, items],
  );
  ...
  async function changeQty(item, nextQty) {
    ...
    try {
      setError('');
      setBusy(true);
      await CartApi.updateItem(item.id, { quantity: nextQty });
      const updated = await CartApi.listByCustomer(customerId);
              ...
              {rows.map((item) => (
                <div key={item.id} className="sb-cart-row sb-cart-row-showcase">
                  <div className="sb-cart-product">
                    <BookCover
                      book={{ display_image_url: item.display_image_url, image_status: item.image_status, title: item.title }}
