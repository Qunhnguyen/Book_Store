...
import { StaffBooksApi, getErrorMessage } from '../../api/client';
...
export default function StaffPage() {
  ...
  async function loadBooks() {
    try {
      setError('');
      const data = await StaffBooksApi.list();
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (editingId) {
        await StaffBooksApi.update(editingId, payload);
      } else {
        await StaffBooksApi.create(payload);
      }
      setForm(initialForm);
      setEditingId(null);
      await loadBooks();
  ...
  async function remove(bookId) {
    try {
      await StaffBooksApi.remove(bookId);
      await loadBooks();
    } catch (err) {
      setError(getErrorMessage(err));
    }
