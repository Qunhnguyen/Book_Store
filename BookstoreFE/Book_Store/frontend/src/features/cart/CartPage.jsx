...
import { CartApi, getErrorMessage } from '../../api/client';
...
export default function CartPage() {
  ...
  async function loadCart() {
    try {
      setError('');
      const data = await CartApi.listByCustomer(customerId);
  ...
  async function ensureCartId() {
    ...
    const created = await CartApi.createCart(Number(customerId));
              ...
              { key: 'quantity', label: 'Qty' },
              {
                ...
                render: (row) => (
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <button onClick={() => increase(row)}>+1 Qty</button>
                    <button onClick={() => removeItem(row)}>Remove</button>
                  </div>
                ),
              },
            ]}
